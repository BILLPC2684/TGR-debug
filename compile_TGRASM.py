  ...
  def prehandle(self,*args):
    ...
    if reg(args[1]) < 0:
      try:
        ...
        raise OpcodeError("invalid operand {}. should be register name or int".format(i))
      self.instruction(0x04,combinetochar(0x00,reg(args[0])),combinetochar(0x0,reg(args[2])),0x00,(res&0xff00)>>8,res&0x00ff)
      return 6
    else:
      self.instruction(0x04,combinetochar(reg(args[1]),reg(args[0])),combinetochar(0x0,reg(args[2])))
      return 6
  ...
  def prehandle(self,*args):
    if len(args) != 2:
      ...
      try:
        ...
        raise OpcodeError("invalid operand {}. should be register name or int".format(args[0]))
      self.instruction(0x21,combinetochar(0x00,reg(args[0])),0x00,0x00,(res&0xff00)>>8,res&0x00ff)
    elif len(args) != 3:
      if reg(args[0]) < 0:
        raise OpcodeError("invalid operand {}. should be register name".format(args[0]))
      ...
      if reg(args[2]) < 0:
        raise OpcodeError("invalid operand {}. should be register name".format(args[2]))
      self.instruction(0x22,combinetochar(reg(args[1]),reg(args[0])),combinetochar(0x00,reg(args[2])))
    else:
...
class title(instruction):
  def prehandle(self,*args):
    if len(args) != 1:
     raise OpcodeError("expected 1 operand, got {}".format(len(args)))
    if str(type(args[0])) != "<class 'str'>":
     raise OpcodeError("expected \"<class 'str'>\", got \"{}\"".format(type(args[0])))
    if   len(args[0]) >  8:
      raise OpcodeError("invalid string size, expected 8 chars or less, got {} (note leave out \"s)".format(len(args[0])))
    elif len(args[0]) <  1:
      raise OpcodeError("invalid string size, expected 1 char or more, got {}".format(len(args[0])))
    title = str(args[0]+" "*(len(args[0])-8))
    enTitle = True
    self.instruction()
    return 0
...
#  j = 0
  ...
  if enTitle == True:
   res = ([0x01,0x54,0x47,0x52,title[0],title[1],title[2],title[3],title[4],title[5],title[6],title[7],title[8]] if not checksum else []) + [0x0C,0x00,0x00,(location&0x00ff0000)>>16,(location&0x0000ff00)>>8,location&0x000000ff] + dataobjs
  else:
   res = ([0x00,0x54,0x47,0x52] if checksum else []) + dataobjs #+ [0x0C,0x00,0x00,(location&0x00ff0000)>>16,(location&0x0000ff00)>>8,location&0x000000ff]
